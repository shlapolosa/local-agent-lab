...
def build() -> C4Diagram:
    ...
    d.component("ingress", "z_events", "fabric-ingress", "GREEN 13 Sep (WP12, deployed c33b8a4, exercised: 8 meeting artifacts → 0 working-file records): only a producer\'s DECLARED products\n(ProcessSpec.products) become lab events — a recording, a transcript, a submission record\nstay pointers (BRS p.8); 29 working-file cards declined and their records withdrawn", fill=DONE)
    # workloads
    d.component("intake", "z_work", "wf-fabric · artifact_intake", "GREEN 13 Sep (WP14, deployed, exercised: a library ADR got its owner at C from the owner map): owner and label LOOKED UP at C —\nthe item\'s label, the owner map / the run\'s requester / the item\'s author, unresolved → asked;\nGREEN before: live run 67 s — identify→classify→associate→impact→\nsynthesise (2 decision records)→overlap→draft-review asked;\n1858-span trace", fill=DONE)
    ...
    # semantic layer
    d.component("catalog", "z_sem", "Knowledge Catalog", "GREEN 13 Sep (WP13, deployed, exercised on a screening run: product = process/output/submission, version = run): a lab product is identified by\nprocess/output/subject[/lane] and versioned by the run — a re-run reopens ONE record;\nGREEN before: Postgres rows in Neon (DDL at boot, hnsw over halfvec; an embedder switch\nre-dimensions the index and semantic_reindex refills it from facets), live local + cloud", fill=DONE)
    d.component("graph", "z_sem", "Traceability Graph", "GREEN 13 Sep (WP18, deployed fdc0b6a, exercised: semantic_derive wrote 4 relatedTo edges at D, queryable): rung D is WRITTEN by two rules after every\npublish (related-to context through a reference; a synthesised record inherits its minutes\' context), each\nderived edge with prov:wasDerivedFrom; GREEN before: edges at C/X/S, SHACL refused a body and a\nguessed owner, impact skipped S, promote S→H by a named\nperson, N-Quads shadow written", fill=DONE)
    ...
    d.component("facade", "z_sem", "Facade", "GREEN 13 Sep (WP15, deployed; semantic_recommend granted and answering; overlap item awaits a real near-duplicate): a draft this close to a PUBLISHED record is an\noverlap item the person adjudicates (keep | duplicate-of); semantic_recommend = published only, with owners;\nGREEN before: embed (nomic, 768-d) · search · similar\nranked the ADR above the minutes for a claims-bus query; the\nlocal stack has no embedder, so local similarity is by test", fill=DONE)
    # gate & consumers
    d.component("approvals", "z_gate", "workflow-mcp approvals + curator", "GREEN 13 Sep (WP16, deployed; impact-notice kind live in approvals_list; awaits a real re-versioned dependency): a new version that published records reference raises\nONE impact-notice to their owners through the same gate (every channel, no new outbound path);\nGREEN before: draft-review asked live with 2 drafts attached,\napproved by a named person, curator promoted the type to H,\nrunner bound approval_id and released the publish run", fill=DONE)
    d.component("projector", "z_gate", "fabric-projector", "GREEN 13 Sep (WP19, deployed, exercised: fabric-metrics.md written 07:11Z, semantic_metrics answered with age): fabric-metrics.md — auto-association, approved\nwithout rewrite, impact acknowledged, duplicate rate, labelled/owned — on the reconciler\'s tick, and\nsemantic_metrics for the bot; GREEN before: wrote 01-sanity-3-csv.md into the pilot library's\nroot via collab_put after the publish run; tagged in the\nloop-guard memory", fill=DONE)
