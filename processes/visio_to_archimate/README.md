...
mode (or `python shared/approvals.py approve <id>`). `python shared/workflows.py list` shows requests.

**One-shot / CLI (dev, demo, smoke):**
```bash
set -a && source .env && set +a
# a path (local tools):
.venv/bin/python -m processes.visio_to_archimate.host [diagram.vsdx|image.png] [-r doc.docx ...]
# or by reference (upload once, read through storage-mcp):
.venv/bin/python -m processes.visio_to_archimate.inputs upload diagram.png req.docx   # -> art:// refs
.venv/bin/python -m processes.visio_to_archimate.host art://<id>/diagram.png -r art://<id>/req.docx
# or publish an event and let the consumer run it:
.venv/bin/python shared/workflows.py request visio_to_archimate art://<id>/diagram.png art://<id>/req.docx
```
...
in `shared/artifacts.py` is ready; until then cloud uploads land in Postgres too, transparently.
...
- Shared parsing + the image sizing contract live in `shared/docparse.py`; the Visio reader is
  `.claude/skills/visio-reader/scripts/read_vsdx.py` (registered in LiteLLM).
