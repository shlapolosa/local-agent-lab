...
class Ontology(Protocol):
    """What every Registry entry provides, whatever its KIND — a metamodel `Vocabulary` (classes,
    relations, permitted matrix) or a SKOS reference `SkosScheme` (a concept hierarchy): a registry
    name, an IRI base, its namespace, its RDF graph and a one-line summary. Callers never
    type-sniff (`hasattr(v, "classes")`); they ask summary()["kind"]."""
    name: str
    base: str
...
class Vocabulary:
    ...
    def summary(self) -> dict:
        return {"name": self.name, "kind": "metamodel", "classes": len(self.classes),
                "relations": len(self.relations), "permitted_pairs": len(self.permitted)}
...
class Registry:
    """Ontologies of any kind, by name (see the Ontology protocol)."""

    def __init__(self):
        self._v: dict[str, Ontology] = {}

    def add(self, v: Ontology) -> Ontology:
