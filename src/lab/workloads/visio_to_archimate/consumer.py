...
from lab.workloads.visio_to_archimate.host import PROCESS, SERVICE, _shutdown, run_once
...
async def _run(root, req, on_trace):
    ...
    return await run_once(root, req.inputs["diagram"],
                          list(req.inputs.get("requirements") or []), on_trace=on_trace)
...
def _describe(req) -> str:
    ...
    return f'{req.inputs["diagram"]} + {len(req.inputs.get("requirements") or [])} doc(s)'
