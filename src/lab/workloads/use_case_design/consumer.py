"""The long-lived `use_case_design` host: consumes `workflow:requests`, runs, writes back.
...
from lab.workloads.use_case_design.host import PROCESS, SERVICE, run_once


async def _run(root, req, on_trace):
    return await run_once(root, req.inputs["submission_ref"], req.inputs["screening_ref"],
                          req.inputs.get("criticality") or {}, on_trace=on_trace,
...
def _describe(req) -> str:
    ...
    return req.inputs.get("submission_ref", "(none)")
