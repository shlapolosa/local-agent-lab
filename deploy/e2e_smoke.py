...
workload will. Run against any deployment by pointing GW at its public gateway URL:
    ...
    GW=https://gateway-production-120b.up.railway.app \
      ...
      .venv/bin/python deploy/e2e_smoke.py

Reads missing values from .env. Prints a PASS/FAIL line per check and exits non-zero on any failure.
...
redisok = s == 200 and (not isinstance(d, dict) or d.get("status") in (None, "healthy", "connected") )
