...
- **Substrate** = the shared platform plane: `redis` (limiter state + streams, internal),
  `gateway` (governance, public), `semantic-mcp` + `adoit-mcp` + `storage-mcp` + `workflow-frontdoor` +
  ...
  The approval gate has TWO faces: the review app for a person with a browser, and `workflow-frontdoor`'s
...
- **Least privilege (per-role env allowlist):** a service receives ONLY the `.env` keys its role
  ...
  workflow-frontdoor / graph-mcp / review / telegram / teams / workload; glob patterns, each line cites the
...
- **Networking:** the substrate lives in one Railway project/environment, so the gateway reaches the
  ...
  `storage-mcp…:9300`, `workflow-frontdoor…:9400`, `graph-mcp…:9500`). The MCP servers get **no public domain**; only `gateway` (targetPort 4000)
