...
#   Usage: set -a && source .env && set +a && gateway/register_skill.sh <team_id> [skill] [env_var]
#   skill    directory under .claude/skills/  (default: archimate-adoit)
...
(cd "$ROOT/.claude/skills" && zip -qr "$ZIP" "$SKILL" -x "*/__pycache__/*" "*.pyc")
