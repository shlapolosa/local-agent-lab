...
def test_runs_board_rows_selection_timeline_and_highlighted_graph():
    ...
    APP._runs_board()
    board = [a[0] for p, a, _ in st.calls if p.endswith("dataframe")][0]
    assert [r["run"] for r in board] == ["run-1", "run-0"]
    assert board[0] == {"run": "run-1", "process": "visio_to_archimate", "input": "sys.vsdx", "status": "running",
                        "current node": "ba (start)", "started": "2026-09-03 10:00:00", "elapsed": "42s",
                        "trace": APP.JAEGER + "ff" * 16}
    ...
    timeline = [a[0] for p, a, _ in st.calls if p.endswith("dataframe")][1]
