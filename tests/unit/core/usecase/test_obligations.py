...
def test_the_baseline_applies_at_every_class_including_the_lowest():
    baseline = {o.guardrail for o in mandatory_for(0, 0, mapping_rows=MAPPING) if o.guardrail}
...
def test_a_higher_exposure_class_inherits_the_one_below_it():
    ...
    e1 = {o.guardrail for o in mandatory_for(1, 0, mapping_rows=MAPPING)}
    e2 = {o.guardrail for o in mandatory_for(2, 0, mapping_rows=MAPPING)}
    e3 = {o.guardrail for o in mandatory_for(3, 0, mapping_rows=MAPPING)}
    assert e1 < e2 < e3
    assert "G02" in e3, "E3 must still carry E1's tool scoping"


def test_influence_classes_inherit_independently_of_exposure():
    i3 = {o.guardrail for o in mandatory_for(0, 3, mapping_rows=MAPPING)}
...
def test_an_obligation_without_a_guardrail_id_is_still_an_obligation():
    ...
    narrative = [o for o in mandatory_for(0, 1, mapping_rows=MAPPING) if not o.guardrail]
...
def test_a_predicate_that_fires_puts_its_guardrail_in_the_set():
    ...
    fired = triggered_for(Workflow(steps=(step,)), "s1", conditions=ANSWERS, guardrails=GUARDRAILS)
...
def test_a_predicate_that_does_not_fire_stays_out():
    ...
    assert "G09" not in triggered_for(Workflow(steps=(step,)), "s1", conditions=ANSWERS, guardrails=GUARDRAILS)


def test_the_always_guardrails_fire_for_every_step():
    fired = triggered_for(Workflow(steps=(s(),)), "s1", conditions=ANSWERS, guardrails=GUARDRAILS)
...
def test_an_unanswered_named_condition_refuses_the_whole_step():
    ...
    with pytest.raises(ObligationError) as e:
        triggered_for(Workflow(steps=(s(activity="retrieve"),)), "s1", conditions={}, guardrails=GUARDRAILS)
...
def test_the_workflow_level_guardrail_reads_the_workflow():
    ...
    assert "G19" not in triggered_for(quiet, "s1", conditions=ANSWERS, guardrails=GUARDRAILS)
    assert "G19" in triggered_for(loud, "s1", conditions=ANSWERS, guardrails=GUARDRAILS)
...
def test_derive_returns_a_control_requirement_set_per_step():
    ...
    out = derive(wf, conditions=ANSWERS, guardrails=GUARDRAILS, mapping_rows=MAPPING)
...
def test_derive_merges_triggered_and_mandated_without_duplicating():
    ...
    ids = [o.guardrail for o in derive(wf, conditions=ANSWERS, guardrails=GUARDRAILS, mapping_rows=MAPPING).by_step["s1"] if o.guardrail]
...
def test_every_obligation_names_the_guardrail_or_the_text_it_came_from():
    wf = Workflow(steps=(s(),))
    for o in derive(wf, conditions=ANSWERS, guardrails=GUARDRAILS, mapping_rows=MAPPING).by_step["s1"]:
