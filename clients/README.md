...
## The two values that change per deployment
- **Base URL**: `http://127.0.0.1:4000` for a local clone; your APIM endpoint in production.
- **Audience** (JWT path only): `ENTRA_GATEWAY_AUDIENCE` from `.env` — the lab-gateway app-id URI.
  Committed templates use this lab's value; a different Entra tenant substitutes its own
  (run `gateway/entra_provision.py`, then `gateway/entra_dev_provision.py`, and re-`az login`).

## Claude Code
Copy `clients/claude-code/settings.json` to your project's `.claude/settings.json`
