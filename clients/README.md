...
- **`ENTRA_GATEWAY_AUDIENCE`** (JWT path): the lab-gateway app-id URI. A different Entra tenant
  substitutes its own in `.env` (run `gateway/entra_provision.py` + `gateway/entra_dev_provision.py`,
...
1. Render the settings for your environment: `./lab.sh clients` (interpolates `GATEWAY_URL`
   and `ENTRA_GATEWAY_AUDIENCE` from `.env` into `clients/claude-code/settings.json`). This runs
...
2. Copy the rendered `clients/claude-code/settings.json` to your project's `.claude/settings.json`
